...
"5", "a0A" and "pass007". Thus the longest password is "pass007" and its length is 7. Note that neither "test" 
nor "?xy1" is a valid password, because "?" is not an alphanumerical character and "test" contains an even number of digits (zero).
...
def solution(S):
    listOfPasswords = S.split()

    maxLength = -1

    for password in listOfPasswords:
        if password.isalnum():
            numCount = sum(1 for ch in password if ch.isdigit())
            charCount = sum(1 for ch in password if ch.isalpha())
            if numCount % 2 == 1 and charCount % 2 == 0:
                maxLength = max(len(password), maxLength)
