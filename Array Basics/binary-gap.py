...
so its longest binary gap is of length 5. Given N = 32 the function should return 0, because N has binary representation 
...
def decimalToBinary(N) -> str:
    binaryArray = []
    while N>0:
        ele = N%2
        binaryArray.append(str(ele))
        N=N//2
...
def solution(N):
    ...
    for chBit in binaryString:
        if chBit == '0':
            currChCount +=1
        else:
            if currChCount > maxChCount:
                maxChCount = currChCount
